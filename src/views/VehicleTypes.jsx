...
const EMPTY = { groupID: '', code: '', name: '', desc: '' };

function validate(form) {
  const e = {};
  if (!form.groupID)              e.groupID = 'Group is required';
  if (!form.code.trim())          e.code    = 'Code is required';
  else if (form.code.length > 10) e.code    = 'Maximum 10 characters';
  if (!form.name.trim())          e.name    = 'Vehicle Type Name is required';
  else if (form.name.length > 50) e.name    = 'Maximum 50 characters';
  return e;
}

export default function VehicleTypes() {
  const [data, setData]               = useState(MOCK.vehicleTypes);
  ...
  const open = (row) => {
    setForm(row
      ? { groupID: row.groupID ?? '', code: row.code, name: row.name, desc: row.desc ?? '' }
      : EMPTY
    );
    setErrors({});
    setModal(row ?? 'add');
  ...
  return (
    <>
      <ListingPage
        idPrefix={ID}
        title="Vehicle Types"
        subtitle="Vehicle type classifications defined at group level"
        columns={[
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div className="form-group" style={{ flex: 1, marginBottom: 0 }}>
              <input
                ...
                placeholder="Search by code, name, description or group…"
                value={search}
      ...
      {modal && (
        <FormModal
          idPrefix={ID}
          title={modal === 'add' ? 'Add Vehicle Type' : 'Edit Vehicle Type'}
          subtitle={modal === 'add' ? 'Create a new vehicle type for a group' : `Editing: ${modal.name}`}
          onClose={close}
          onSave={save}
          isEdit={modal !== 'add'}
        >
          ...
          <div className="form-grid form-grid-1" style={{ gridTemplateColumns: '1fr' }}>
          ...
          {/* Type details */}
          <p className="section-label">Type Details</p>
          <div className="form-grid form-grid-2">
            <div className="form-group">
              ...
              {errors.code
                ...
                : <span className="field-hint">Unique identifier, max 10 characters</span>}
            </div>

            <div className="form-group">
              <label className="form-label">Vehicle Type Name <span className="required">*</span></label>
              <input
                id={`${ID}-input-name`}
              ...
              {errors.name
                ...
                : <span className="field-hint">{form.name.length}/50 characters</span>}
            </div>
            ...
            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
              ...
              <textarea
                ...
                placeholder="Optional: brief description of this vehicle type"
                rows={3}
              />
              <span className="field-hint">Optional — helps users identify the correct type</span>
            </div>
          </div>
        </FormModal>
      )}
          ...
          <div className="card fade-up" style={{ width:'100%', maxWidth:420 }}>
            <div className="card-header">
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                ...
                <div>
                  <div className="card-header-title">Delete Vehicle Type</div>
